...
- Signature preview/result are WYSIWYG (2026-08-22, see DECISIONS D027–D029): upload preview renders the trimmed canvas (not the raw scan) so small signatures no longer float in white; the result matte is capped to the displayed image width (`max-width: min(340px,100%)` + caption removed) so the white card hugs the image and previews show no artificial side padding. The draw canvas is opaque white and the pipeline flattens to white, fixing black-on-black in dark mode. 160 tests passing.
