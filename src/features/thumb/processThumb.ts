...
/**
 * Thumb-impression pipeline: trim empty margins → fit resize (never upscale) →
 * encode → optimize → validate.
 *
 * Mirrors signature semantics (TR-022): `within` dimension mode, allowedScales
 * when a file-size constraint exists — the only recourse for PNG where quality
 * is ignored. Thumb prints are typically square ink impressions on white
 * paper; guidance is upload-centric (no draw canvas).
 */
const THUMB_ALLOWED_SCALES = [1, 0.9, 0.8, 0.7, 0.6, 0.5]

/**
 * Serializable thumb pipeline; identical code in-thread and in-worker (T018/T022).
 * Returns everything except the session object URL.
 */
export async function computeThumbOutput(input: {
  ...
  const optimization = await optimizeEncoding(
    createCanvasEncoder(flattened as unknown as EncodableCanvas, format),
    {
      ...
      allowedScales: input.profile.fileSize ? THUMB_ALLOWED_SCALES : undefined,
    },
  )
  ...
  const baseName = input.fileName.replace(/\.[^.]+$/, '') || 'thumb'
...
export async function processThumb(input: {
  source: DrawableSource
  profile: ImageRequirements
  fileName: string
